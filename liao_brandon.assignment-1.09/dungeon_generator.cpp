...
void inventory_list(struct Dungeon * dungeon, bool is_equip) {
    ...
    while(!exit) {
        ...
        for(int i = 0; i < slot_count; i++) {
            ...
            mvwprintw(win, i+1, 1, "%c) %s", base_char + i, 
                     present ? item->n.c_str() : "Empty");
        }
...
void process_pc_input(struct Dungeon * dungeon, Bool * run, Bool * regen) {
	...
	setSpriteAT(dungeon->ss, 0, getSpriteAT(dungeon->ss, 0) + (100 / getSpriteASS(dungeon->ss, 0) + spbns));
...
int main(int argc, char *argv[]) 
{
	...
	while(run == TRUE) {
        ...
        mvprintw(0, 0, "PC HP: %d   ", dungeon.plyr->s.hp);
        refresh();

        //check for lowest turn count (alive sprites only)
        int l = -1;
        for (i = 0; i < dungeon.ns; i++) {
            if (getSpriteAA(dungeon.ss, i)) {
                l = i;
                break;
            }
        }
        if (l == -1) break; //all sprites are dead

        for (i = l + 1; i < dungeon.ns; i++) {
            if (getSpriteAA(dungeon.ss, i) && getSpriteAT(dungeon.ss, i) < getSpriteAT(dungeon.ss, l)) {
                l = i;
            }
        }
