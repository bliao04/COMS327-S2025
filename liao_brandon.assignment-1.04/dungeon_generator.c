...
struct Dungeon init_dungeon(int h, int w, int mr) {
	...
	dungeon.ss = calloc(dungeon.ms, sizeof(struct Sprite));

	//djikstra-based cost map allocation 
	dungeon.cst = calloc(w*h, sizeof(int *));
	for(i = 0; i < dungeon.h; i++) {
		dungeon.cst[i] = calloc(dungeon.w, sizeof(int));
	}

	//djikstra-based cost map allocation
	dungeon.csnt = calloc(w*h, sizeof(int *));
	for(i = 0; i < dungeon.h; i++) {
		dungeon.csnt[i] = calloc(dungeon.w, sizeof(int));
	}
...
int main(int argc, char *argv[]) 
{
	...
	//generate initial moves for all sprites in the dungeon
	for(i = 0; i < dungeon.ns; i++) {
		generate_next_move(&dungeon, i);
	}

	//main game loop
	bool first = TRUE; //flag to track the first iteration
	bool run = TRUE; //flag to control game loop
	while(run == TRUE) {
		int l = 0; //index of the sprite with the smallest turn counter

		//find the sprite with the smallest turn counter
		for(i = 0; i < dungeon.ns; i++) {
			if(dungeon.ss[i].t < dungeon.ss[l].t) {
				l = i; //update the index of the sprite with the smallest turn counter
			}
		}
		//parse and execute the move for the selected sprite
		parse_move(&dungeon, l);
		generate_next_move(&dungeon, l);

		//if the pc moves or it's the first iteration, update and display the dungeon
		if(l == dungeon.pcSS || first == TRUE) {
			map_dungeon_nont(&dungeon); //update the non-tunneling map
			map_dungeon_t(&dungeon); //update the tunneling map
			print_dungeon(&dungeon, 0, 0); //display the dungeon
			sleep(3);
		}
		...
		if(any == FALSE) {
			printf("You Win!\n"); //player wins if no monsters are left
			goto END;
		}
		first = FALSE; //set the first iteration flag to false after the first loop
	}

	//display the final state of the dungeon and announce the player's loss
