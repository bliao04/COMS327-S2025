...
#ifndef CLAMP
#define CLAMP(val, min, max) ((val) < (min) ? (min) : ((val) > (max) ? (max) : (val))) //clamp macro
#endif
...
#define dungeon_generator
		...
		public:
			...
			Item eqs[12]; //equipment slots
			Item inv[10]; //inventory slots
			bool eqsp[12]; //equipment slot flags
			bool invp[10]; //inventory slot flags

			int max_hp; //max hp for PC
			int mana; //current mana for PC
			int max_mana; //max mana for PC
		};
		...
		class MonFac {
		public:
			int dn; //definition count 
			SpriteTemp * md; //template for monster generation
			static std::set<std::string> killed_uniques; //track dead uniques
			
			MonFac(int, SpriteTemp *); //parameterized constructor
			MonFac(void); //default constructor
			Sprite* GetMon(Dungeon *dungeon); //generates and returns a new monster for the given dungeon
			static void MarkUniqueDead(const std::string& name); //marks a unique monster as dead by adding to killed_uniques set
		};
			
		class ObjFac {
		public:
			int dn; //definition count
			ItemTemp * id; //template for item generation
			...
			ObjFac(int, ItemTemp *); //parameterized constructor
			ObjFac(void); //default constructor 
			Item GetObj(); //generates and returns a new item
		    static void MarkArtifactObtained(const std::string& name); //marks an artifact as obtained by the player by adding to obtained_artifacts set
		};		
		...
		public:
			...
			int look_mode; //PC look state
			int ranged_mode; //PC ranged attack state
