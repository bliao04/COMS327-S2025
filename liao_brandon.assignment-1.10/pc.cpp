...
PC * initPC(Dungeon * dungeon) {
	...
	p->n = "Player";
	p->dl = 3;
    p->desc = new std::string[p->dl];
    p->desc[0] = "The brave adventurer who entered the dungeon.";
    p->desc[1] = "Equipped with basic gear and a thirst for adventure.";
    p->desc[2] = "The fate of the dungeon rests in their hands.";
	p->sn = 0;
	p->a = TRUE;

	//initialize equipment and inventory slots to empty
	for (int i = 0; i < 12; i++) {
		p->eqs[i] = Item();  
		p->eqsp[i] = false;   
	}
