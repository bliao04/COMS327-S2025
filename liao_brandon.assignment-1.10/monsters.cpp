...
/* helper function to check if a tile is occupied by a NPC */
Bool has_NPC(Dungeon *d, int x, int y) {
    //check PC
    if (getSpriteAPX(d->ss, d->pcSS) == x && 
        getSpriteAPY(d->ss, d->pcSS) == y) {
        return TRUE;
    }
    
    //check NPCs
    for (int i = 1; i < d->ns; i++) {
        if (getSpriteAPX(d->ss, i) == x && 
            getSpriteAPY(d->ss, i) == y) {
            return TRUE;
        }
    }
    return FALSE;
...
Bool test_loc(Dungeon * dungeon, int x, int y, Sprite *s) {
    //first check PC position
    if (x == getSpriteAPX(dungeon->ss, dungeon->pcSS) && 
        y == getSpriteAPY(dungeon->ss, dungeon->pcSS)) {
        return FALSE; //can't move into PC's cell directly
    }

    //check for other NPCs
    for (int i = 1; i < dungeon->ns; i++) {
        if (i != s->sn && 
            getSpriteAPX(dungeon->ss, i) == x && 
            getSpriteAPY(dungeon->ss, i) == y) {
            
            //attempt displacement
            int dx[] = {-1,0,1,-1,1,-1,0,1};
            int dy[] = {-1,-1,-1,0,0,1,1,1};
            
            //check all 8 directions for empty cell
            for (int dir = 0; dir < 8; dir++) {
                int nx = x + dx[dir];
                int ny = y + dy[dir];
                
                //validate new position
                if (nx >= 0 && nx < dungeon->w &&
                    ny >= 0 && ny < dungeon->h &&
                    dungeon->d[ny][nx].hardness == 0 &&
                    !has_NPC(dungeon, nx, ny)) {
                    
                    //move the occupant
                    setSpriteAPX(dungeon->ss, i, nx);
                    setSpriteAPY(dungeon->ss, i, ny);
                    return TRUE;
                }
            }
            
            //if no empty space, swap positions
            setSpriteAPX(dungeon->ss, i, s->p.x);
            setSpriteAPY(dungeon->ss, i, s->p.y);
            return TRUE;
        }
    }
    ...
    if (x > 0 && x < dungeon->w-1 && 
        y > 0 && y < dungeon->h-1) {
        return dungeon->d[y][x].hardness < 255 && 
               (getSpriteSTu(s) || dungeon->d[y][x].hardness == 0);
    }
    return FALSE;
}
...
void generate_next_move(Dungeon * dungeon, int sn) {
	...
	int monster_speed = getSpriteSS(s);
    if (monster_speed <= 0) monster_speed = 1; //safety check
    setSpriteAT(dungeon->ss, sn, getSpriteAT(dungeon->ss, sn) + (100 / monster_speed));
...
void parse_move(Dungeon * dungeon, int sn) {
	//store old position
	int old_x = getSpriteAPX(dungeon->ss, sn);
	int old_y = getSpriteAPY(dungeon->ss, sn);

	//update to new position
	setSpriteAPX(dungeon->ss, sn, getSpriteAToX(dungeon->ss, sn));
	setSpriteAPY(dungeon->ss, sn, getSpriteAToY(dungeon->ss, sn));

	//check if monster moved into PC's cell
	if (getSpriteAPX(dungeon->ss, sn) == getSpriteAPX(dungeon->ss, dungeon->pcSS) && 
		getSpriteAPY(dungeon->ss, sn) == getSpriteAPY(dungeon->ss, dungeon->pcSS)) {
		...
		//check for death
		if (dungeon->plyr->s.hp <= 0) {
			dungeon->go = true;
			setSpriteAA(dungeon->ss, dungeon->pcSS, false);
		}
		
		//move monster back to original position
		setSpriteAPX(dungeon->ss, sn, old_x);
		setSpriteAPY(dungeon->ss, sn, old_y);
	}
}
