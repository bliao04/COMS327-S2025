...
/* generate next move for monster using dungeon cost maps */
...
/* commit generated move to sprite position */
...
/* add sprite to dungeon with dynamic array expansion */
void add_sprite(Dungeon * dungeon, Sprite * s) {
    ...
    if (dungeon->ns >= dungeon->ms) {
        ...
        Sprite *new_ss = new Sprite[new_capacity];  // Use new[] instead of realloc
        
        //copy existing sprites
        for (int i = 0; i < dungeon->ns; i++) {
            new_ss[i] = dungeon->ss[i];  // This uses the assignment operator
        }
...
/* check win condition, any alive monsters */
