...
int parse_monsters(Dungeon * dungeon) {
    ...
    dungeon->mm = 100; //max monsters (arbitrary 100 because thats what worked)
    vector<Monster> mons; //temporary monster storage
    
    string line;
    ifstream md(path);
    if(md.is_open()) {
        Monster b_mo = {0}; //base monster template
        Monster mo; //current monster being parsed
        while(getline(md, line)) {            
            if(first) {
                ...
                if(line == "BEGIN MONSTER") {
                    ...
                    //parse speed
                    mo.s.s = rolldie(line.substr(6));
                } else if(line.find("HP") != string::npos) { 
                    //parse hit points
                    mo.s.hp = rolldie(line.substr(3));
...
int parse_items(Dungeon * dungeon) {
    ...
    int di = 0; //item count
    bool first = true; //flag for first line check
    dungeon->mi = 100; //max items (arbitrary 100)
    std::vector<Item> items; //temporary item storage
    
    std::string line;
    std::ifstream od(path);
    if(od.is_open()) {
        Item b_it{}; //base item template
        Item it; //current item being parsed
        while(getline(od, line)) {
            if(first) {
                ...
                if(line == "BEGIN OBJECT") {
                    ...
                    //parse weight
                    it.w = rolldie(line.substr(7));
                }
                else if(line.rfind("HIT ", 0) == 0) {
                    //parse hit bonus
                    it.hib = rolldie(line.substr(4));
                }
                    ...
                    it.sa = rolldie(line.substr(5));
                }
                else if(line.rfind("VAL ", 0) == 0) {
                    //parse value
                    it.v = rolldie(line.substr(4));
                }
                else if(line.rfind("DODGE ", 0) == 0) {
                    //parse dodge bonus
                    it.dob = rolldie(line.substr(6));
                }
                else if(line.rfind("DEF ", 0) == 0) {
                    //parse defense bonus
                    it.deb = rolldie(line.substr(4));
                }
                else if(line.rfind("SPEED ", 0) == 0) {
                    //parse speed bonus
                    it.spb = rolldie(line.substr(6));
                }
...
int main(int argc, char *argv[]) 
{
	...
	parse_items(&dungeon);
	print_monster_descriptions(&dungeon);
	print_item_descriptions(&dungeon);
	return 0;
	...
	Sprite *pc = generate_sprite(&dungeon, '@', -1, -1, 1);
	add_sprite(&dungeon, pc);

	//generate and add monster sprites to the dungeon
	int i;
	for(i = 0; i < num_mon; i++) {
		Sprite *m = generate_sprite(&dungeon,'m' , -1, -1, 1);
		//m.sn = i;
		setSpriteSn(m, i);
